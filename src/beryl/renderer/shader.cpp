...
uint32_t beryl::renderer::Shader::Compile(uint32_t type, const std::string& source)
{
...
beryl::renderer::Shader::~Shader() 
{
    if (m_shader_id != 0)
        glDeleteProgram(m_shader_id);
}

void beryl::renderer::Shader::Create(const std::string& vertex_src, const std::string& fragment_src)
{
    if (m_shader_id != 0)
        glDeleteProgram(m_shader_id);
...
void beryl::renderer::Shader::Use() const
{
    if (m_shader_id != 0)
        glUseProgram(m_shader_id);
}

uint32_t beryl::renderer::Shader::GetShaderId() const { return m_shader_id; }